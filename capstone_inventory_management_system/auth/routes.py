    ...
    if request.method == 'POST':
        ...
        if not user or not user.verify_password(password):
            ...
            if email == "admin@nucleusteq.com":
                ...
                current_app.logger.info('Admin logged in: %s', msg)
                ...
                current_app.logger.info('Employee logged in: %s', msg)
...
def assign_item():
    if 'loggedin' in session:
        ...
        if item:
            if assigned_to_id:
                # check if the item with the same name is assigned to the user
                ...
                current_app.logger.info('Item assigned successfully: %s to user %s', item.name, assigned_to_id)
        else:
